...
import { FoodCategoriesProps } from '../../types/mapSearchProps';

const CategoriesPrefs: React.FC<FoodCategoriesProps> = ({
  handleCategoriesChange,
  currCategories,
}) => {
  const [tags, setTags] = useState<string[]>(currCategories);
  ...
  return (
    <div>
      ...
      {filteredCategories.length > 0 && (
        <ul className="block w-full shadow-sm  py-1.5 bg-inherit border border-gray-200 text-gray-900 sm:text-sm sm:leading-6 max-h-[100px] overflow-y-auto rounded-b-md">
          {filteredCategories.map((category, index) => (
            <div
              key={index}
              onClick={() => handleCategoryClick(category)}
              className="border-0 py-1 px-2 on hover:bg-gray-100 cursor-pointer"
            >
              <li className="px-2 text-gray-700 bg-indigo-200 w-fit rounded-full py-1 text-xs">
                {category}
              </li>
            </div>
          ))}
